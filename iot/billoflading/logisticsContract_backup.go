...
"github.com/hyperledger/fabric/core/chaincode/shim"
    "github.com/hyperledger/fabric/core/util"   //for v0.6
...
func (t *SimpleChaincode) registerBillOfLading(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) { 
   ...
   // Prepare the record to put Bill of Lading Registration information in the stub
    ...
    invokeArgs:= util.ToChaincodeArgs(f, string(regJSON))
	_, err = stub.InvokeChaincode(containerChainCode, invokeArgs)
	...
	invokeArgs = util.ToChaincodeArgs(f, string(compJSON))
    _, err = stub.InvokeChaincode(complianceChainCode, invokeArgs)
